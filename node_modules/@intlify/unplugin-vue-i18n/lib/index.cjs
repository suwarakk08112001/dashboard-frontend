...
function resourcePlugin({
  ...
  function intlifyVirtualize(realPath) {
    const normalized = pathe.normalize(realPath);
        ...
        if (meta.framework === "vite" && !hasViteJsonPlugin) {
          if (id.includes("?vue&type=i18n") && /[?&]lang\.(?:json|json5)(?:$|&)/.test(id)) {
            ...
            if (query.src && importer) {
              const importerPath = (isIntlifyVirtualId(importer) ? virtualIdToRealPath.get(importer) : importer).split("?")[0];
              if (importerPath.endsWith(".vue")) {
                const resolvedSidecar = pathe.resolve(pathe.dirname(importerPath), filename);
          ...
          if (idPath.startsWith(".")) {
            ...
            resolvedPath = pathe.resolve(pathe.dirname(realImporter), idPath);
        ...
        if (isIntlifyVirtualId(id)) {
          ...
          if (realId.includes("?vue&type=i18n")) {
            ...
            if (query.src) {
              ...
              source = await node_fs.promises.readFile(filename, "utf-8");
            } else {
              this.addWatchFile(filename);
              const sfcSource = await node_fs.promises.readFile(filename, "utf-8");
            ...
            const generate2 = getGenerator(langInfo2, bundleUtils.generateYAML);
          ...
          const code = await node_fs.promises.readFile(realId, "utf-8");
          const langInfo = pathe.parse(realId).ext;
...
function resolveBlockSrc(src, sfcFilename) {
  if (src.startsWith(".") || !src.startsWith("/") && !/^[a-z]:[/\\]/i.test(src)) {
    return pathe.resolve(pathe.dirname(sfcFilename), src);
  }
  return src;
}
